...
export async function runOfficial({target, task, decide, maxSteps=20, maxMs=60000, emit=()=>{}}) {
  ...
  const read = async () => pageScope(await target.getAXState({disableDiffing:true,emit:false}));
  const result = (status, extra={}) => ({status, task_ms:performance.now()-started, rows, ...extra});
  let state=await read();
  for(let step=0;step<=maxSteps;step++) {
    if(!new RegExp(task.scope,'mu').test(state)) return result('scope_changed');
    ...
    try { decision=await decide({goal:task.goal,state:context,history:history.slice(-4),candidates:candidates.map(({id,op,description,text,key})=>({id,op,description,text,key}))}); }
    ...
    const row={step,decision_ms:performance.now()-t,choice:decision.id,probability:decision.probability};
    if(performance.now()-started>=maxMs)return result('time_budget');
    ...
    if(!new RegExp(task.scope,'mu').test(fresh))return result('scope_changed');
    ...
    } catch(e) {return result('action_failed',{error:e.message});}
