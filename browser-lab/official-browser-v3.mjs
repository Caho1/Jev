...
export async function runOfficial({target, task, decide, maxSteps=20, maxMs=60000, emit=()=>{}}) {
  ...
  for(let step=0;step<=maxSteps;step++) {
    ...
    let decision;
    try { decision=await decide({goal:task.goal,state,history:history.slice(-4),candidates:candidates.map(({id,op,description,text,key})=>({id,op,description,text,key}))}); }
