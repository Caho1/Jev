"""在本地 MPS 微调 Laya 控件评分；训练、完整候选开发检查与历史客服权重分开保存。"""
    ...
    with target.open("x") as stream:
        for frame_index, frame in enumerate(frames):
            ...
            for at in range(0, len(order), args.eval_batch):
                ...
                logits, _ = model(**batch)
                values = (logits[:, 1]-logits[:, 0]).float().cpu().numpy()
...
def run(args, event):
    ...
    if not torch.backends.mps.is_available():
        raise RuntimeError("本机 MPS 不可用，未切换 CPU 或远程运行")
    torch.mps.set_per_process_memory_fraction(args.memory_limit_gib*2**30/torch.mps.recommended_max_memory())
...
def main():
    ...
    args.device, args.precision, args.mode, args.seq_len = "mps", "fp32", "lora", 8192
